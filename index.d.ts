declare module "sdl2" {
    ...
    function init(flags: number): number;
    function createWindowAndRenderer(width: number, height: number, flags: number): number;
    function setRenderDrawColour(red: number, green: number, blue: number, alpha: number): number;
    ...
    function renderDrawPoint(x: number, y: number): number;

    function delay(ms: number): void;

    function pollEvent(): number;
